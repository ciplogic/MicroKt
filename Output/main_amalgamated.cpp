...
void shellSort2(MutableList<SymbolInfo> list, SortyFunc sortyFunc);
...
TResult<SkeletonNode> foldFunction(Ref<Scanner> scanner, List<Token> modifiers) {
  ...
  fun Scanner -> linesTokens ( ) : MutableList < Token > {   auto scanner = this  auto lineTokens = mutableListOf < Token > ( )  while ( ! scanner -> isAtEnd ( ) ) {   auto peek = scanner -> peek ( ) -> value !!  if ( peek -> type == TokenType -> EOLN ) break  lineTokens -> add ( scanner -> advance ( ) )  scanner -> advance ( )  return lineTokens;
  fun tokensToStatement ( tokens : List < Token > ) : SkeletonNode {   auto node = SkeletonNode ( SkeletonType -> STATEMENT )  for ( t in tokens ) {   node -> children -> add ( SkeletonNode ( SkeletonType -> ATOM , t ) )  return node;
  fun foldFunctionHeader ( scanner : Scanner , modifiers : List < Token > ) : TResult < SkeletonNode > {   auto node = SkeletonNode ( SkeletonType -> FUNCTION )  auto modifierNodes = modifiersToAtoms ( modifiers )  node -> children -> addAll ( modifierNodes )  auto keyword = scanner -> advance ( )  node -> children -> add ( SkeletonNode ( SkeletonType -> ATOM , keyword ) )  while ( ! scanner -> isAtEnd ( ) ) {   auto peekRes = scanner -> peek ( )  if ( peekRes -> isError ( ) ) return peekRes -> asError ( )  auto t = peekRes -> value !!  if ( t -> value == "{" || t -> value == "=" ) break  if ( t -> type == TokenType -> EOLN ) {   node -> children -> add ( SkeletonNode ( SkeletonType -> ATOM , scanner -> advance ( ) ) )  continue  auto child = parseHeaderNext ( scanner )  if ( child -> isError ( ) ) return child  node -> children -> add ( child -> value !! )  return success ( node );
  private fun foldLineRemainder ( scanner : Scanner ) : TResult < List < SkeletonNode > > {   auto list = mutableListOf < SkeletonNode > ( )  while ( ! scanner -> isAtEnd ( ) ) {   auto peek = scanner -> peek ( ) -> value !!  if ( peek -> type == TokenType -> EOLN ) break  auto res = parseNext ( scanner )  if ( res -> isError ( ) ) {   return error ( res -> errorMessage !! )  list -> add ( res -> value !! )  return success ( list );
}
TResult<SkeletonNode> foldGroupInHeader(Ref<Scanner> scanner, PairRule rule, HeaderParserMapper childParser) {
...
void shellSort(MutableList<SymbolInfo> list, SortyFunc sortyFunc) {
  ...
  while ( gap > 0 ) {   for ( i in gap ..< list -> size ) {   auto rightSideValue = list [ i ]  auto j = i - gap  while ( j > = 0 ) {   auto current = list [ j ]  if ( sortyFunc ( current , rightSideValue ) ) {   swapInList ( list , j , i )  j -= gap  gap -= 1;
}
void shellSort2(MutableList<SymbolInfo> list, SortyFunc sortyFunc) {
  auto gap = list -> size / 2;
...
Boolean isTypeDependentOnOtherType(Ref<SymbolInfo> t1, MiniType typeToSearch) {
  if ( t1 -> type == SkeletonType -> CLASS ) {   auto miniType = t1 -> decl as MiniClass  auto dependentTypes = getDependentTypesOfMiniClass ( miniType )  if ( dependentTypes -> isEmpty ( ) ) {   return false  auto view = dependentTypes -> toListView ( );
  return indexOf != - 1;
}
Boolean areTypesOrdered(Ref<SymbolInfo> t1, Ref<SymbolInfo> t2) {
